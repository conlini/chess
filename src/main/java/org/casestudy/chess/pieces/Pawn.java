...
public class Pawn extends Piece {
    ...
    public boolean canMoveToSquare(Square targetSquare) {
        switch (direction) {
            case Down: {
                if (hasMoved) {
                    int[] ymoves = new int[]{1};
                    int[] xmoves = new int[]{0};
                    ...
                    if (validateCrossMove(targetSquare, ymoves, xmoves)) {
                        return true;
                    }
                    return false;
                    ...
                    int[] ymoves = new int[]{1, 2};
                    int[] xmoves = new int[]{0, 0};
                    for (int i = 0; i < xmoves.length; i++) {
                        Square square = new Square(currentPlace.getX() + xmoves[i], currentPlace.getY() + ymoves[i]);
                        if (MoveUtil.isValidSquare(square) && square.equals(targetSquare)) {
                            return true;
    ...
    private boolean validateCrossMove(Square targetSquare, int[] ymoves, int[] xmoves) {
        int[] crossMoveY = new int[]{1, 1};
        int[] crossMoveX = new int[]{-1, 1};
